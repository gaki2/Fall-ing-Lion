  ...
  Grass.prototype = {
    ...
    paint: function (ctx, time, ambient) {
      ...
      var ang =
        this.angle +
        Math.PI / 2 +
        ((inc_punta_hierba * Math.PI) / 180) *
          (this.maxAngle * Math.cos(time * 0.0002));
      var px =
        this.coords[0] +
        this.offset_control_point +
        this.alto_hierba * Math.cos(ang);
...
function lerp(time, last) {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    for (var j = 0; j < 3; j++) {
      rgb += Math.floor(
        ((colors[i1][i * 3 + j] - colors[i0][i * 3 + j]) * time) / last +
          colors[i0][i * 3 + j]
      );
      if (j < 2) rgb += ",";
    }
...
// sky colors
colors = [
  [0x00, 0x00, 0x3f, 0x00, 0x3f, 0x7f, 0x1f, 0x5f, 0xc0, 0x3f, 0xa0, 0xff],

  [0x00, 0x3f, 0x7f, 0xa0, 0x5f, 0x7f, 0xff, 0x90, 0xe0, 0xff, 0x90, 0x00],

  [0x00, 0x00, 0x00, 0x00, 0x2f, 0x7f, 0x00, 0x28, 0x50, 0x00, 0x1f, 0x3f],

  [0x1f, 0x00, 0x5f, 0x3f, 0x2f, 0xa0, 0xa0, 0x1f, 0x1f, 0xff, 0x7f, 0x00],
];
